...
- 当用户要求检查或执行本地到 GitHub、再到生产服务器的推送、合并和部署时，使用项目 skill `db-push-merge-deploy`。
- 需要写 Git、GitHub 或生产服务器时，只创建一个 `db-release-deployer` 自定义子智能体作为发布单写入者，主智能体等待其完成并汇总结果；不得并行运行多个发布写入智能体。
- 通用代码部署默认沿用线上 `current-artifact`。新数据制品切换、服务器配置修改、会话库恢复、Git 历史重写或不可变 release 删除不包含在默认授权内。
