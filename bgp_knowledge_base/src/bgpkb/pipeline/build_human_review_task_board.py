...
REPORT = paths.report_path("human_review_task_board_report")
JSONL_OUTPUT = DATASET_DIR / "human_review_task_board.jsonl"
CSV_OUTPUT = DATASET_DIR / "human_review_task_board.csv"

GENERATED_BY = "src/bgpkb/pipeline/build_human_review_task_board.py"
...
def session_tasks():
    ...
    for status in load_jsonl(DATASET_DIR / "human_review_session_status.jsonl"):
        ...
        tasks.append({
            ...
            "primary_input": f"data/generated/reports/review/human_review_session_guides/{session_id}.md",
            "secondary_input": f"data/review_inputs/human_review_session_decision_templates/{session_id}_decisions_template.csv",
            "suggested_command": f"python3 -m bgpkb.pipeline.import_human_review_session_decisions --session-id {session_id}",
            "write_command": f"python3 -m bgpkb.pipeline.import_human_review_session_decisions --session-id {session_id} --write",
            "needs_llm": False,
...
def source_tasks(start_order):
    ...
    for offset, source in enumerate(source_records, start=1):
        ...
        tasks.append({
            ...
            "primary_input": "data/generated/reports/review/human_review_source_matrix_report.md",
            "secondary_input": source.get("source_path", ""),
            "suggested_command": "python3 -m bgpkb.pipeline.build_human_review_source_matrix",
            "write_command": "",
            "needs_llm": False,
            "generated_by": GENERATED_BY,
        })
    ...
    return [
        {
            ...
            "primary_input": "data/review_inputs/human_review_decisions.csv",
            "secondary_input": "data/generated/reports/review/human_review_input_validation_report.md",
            "suggested_command": "python3 -m bgpkb.pipeline.build_human_review_input_validation",
            "write_command": "",
            "needs_llm": False,
            "generated_by": GENERATED_BY,
        },
        {
            ...
            "primary_input": "data/review_inputs/human_review_decisions.csv",
            "secondary_input": "data/generated/reports/review/human_review_decision_audit_report.md",
            "suggested_command": "python3 -m bgpkb.pipeline.build_human_review_decision_audit",
            "write_command": "",
            "needs_llm": False,
            "generated_by": GENERATED_BY,
        },
        {
            ...
            "primary_input": "data/derived/datasets/human_review_decision_audit.jsonl",
            "secondary_input": "data/generated/reports/review/human_review_decision_apply_report.md",
            "suggested_command": "python3 -m bgpkb.pipeline.apply_human_review_decisions",
            "write_command": "python3 -m bgpkb.pipeline.apply_human_review_decisions --write",
            "needs_llm": False,
            "generated_by": GENERATED_BY,
        },
    ]
...
def write_report(records):
    lines = [
        ...
        "",
        f"- 任务数：{len(records)}",
        f"- JSONL 输出：`data/derived/datasets/human_review_task_board.jsonl`",
        f"- CSV 输出：`data/derived/datasets/human_review_task_board.csv`",
        "",
        "## 任务清单",
        "",
        "| 顺序 | 类型 | 标题 | 主输入 | 建议命令 |",
