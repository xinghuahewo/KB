...
REVIEW_INPUT = paths.REVIEW_INPUTS_DIR / "human_review_decisions.csv"
REPORT = paths.report_path("human_review_input_validation_report")
...
def build_check(check_order, check_type, severity, checked_count, issues, message, suggested_action, needs_llm=False):
    ...
    return {
        ...
        "needs_llm": needs_llm,
        "generated_by": "src/bgpkb/pipeline/build_human_review_input_validation.py",
    }
...
def build_records():
    ...
    records.append(build_check(
        ...
        "运行 `python3 -m bgpkb.pipeline.build_human_review_decision_template` 初始化模板。",
    ))
    ...
    records.append(build_check(
        ...
        "核对实体 ID；若是新增实体，先按实体 schema 加入 data/knowledge/entities/*.jsonl 并重跑流水线。",
    ))
...
def write_report(records):
    ...
    lines = [
        ...
        "## 范围",
        "",
        "本报告只校验 `data/review_inputs/human_review_decisions.csv` 的结构和机械一致性，不判断实体内容是否应批准或拒绝。",
        "",
        "该步骤不联网、不下载、不调用 LLM、不做语义判断，也不修改 data/knowledge/entities/*.jsonl。",
        "",
        "## 摘要",
        "",
        f"- 校验记录数：{len(records)}",
        ...
        "- JSONL 输出：`data/derived/datasets/human_review_input_validation.jsonl`",
        "- CSV 输出：`data/derived/datasets/human_review_input_validation.csv`",
        "- 人工决策输入：`data/review_inputs/human_review_decisions.csv`",
        "",
        "## 校验项",
        "",
