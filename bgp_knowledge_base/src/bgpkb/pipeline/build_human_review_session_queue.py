...
REPORT = paths.report_path("human_review_session_queue_report")
...
def next_step_for(queue_status):
    if queue_status == "ready_to_apply":
        return "显式运行 src/bgpkb/pipeline/apply_human_review_decisions.py 前，人工再次确认该决策。"
...
def build_records():
    ...
    for index, workbook in enumerate(workbook_records, start=1):
        ...
        records.append({
            ...
            "generated_by": "src/bgpkb/pipeline/build_human_review_session_queue.py",
        })
    return records
...
def write_report(records):
    ...
    lines = [
        ...
        f"- JSONL 输出：`data/derived/datasets/human_review_session_queue.jsonl`",
        f"- CSV 输出：`data/derived/datasets/human_review_session_queue.csv`",
