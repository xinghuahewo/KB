...
REPORT = paths.report_path("human_review_handoff_report")
JSONL_OUTPUT = DATASET_DIR / "human_review_handoff.jsonl"
CSV_OUTPUT = DATASET_DIR / "human_review_handoff.csv"

GENERATED_BY = "src/bgpkb/pipeline/build_human_review_handoff.py"
...
def expected_output_for(task):
    ...
    if task_type == "review_session":
        return (
            "先在 "
            f"data/review_inputs/human_review_session_decision_templates/{session_id}_decisions_template.csv "
            "中填写人工判断，再显式导入到 data/review_inputs/human_review_decisions.csv。"
        )
    if task_type == "review_source":
        return "按来源核验证据后，把实体级人工判断写入 data/review_inputs/human_review_decisions.csv 或对应 session 模板。"
    if task_type == "validate_input":
        return "生成 data/generated/reports/review/human_review_input_validation_report.md 和 data/derived/datasets/human_review_input_validation.*，只校验主决策输入。"
    if task_type == "audit_decisions":
        return "生成 data/generated/reports/review/human_review_decision_audit_report.md 和 data/derived/datasets/human_review_decision_audit.*，只审计不应用。"
...
def verification_command_for(task):
    task_type = task.get("task_type", "")
    if task_type == "review_session":
        return "python3 -m bgpkb.pipeline.build_human_review_session_status"
    if task_type == "review_source":
        return "python3 -m bgpkb.pipeline.build_human_review_source_matrix"
    if task_type == "validate_input":
        return "python3 -m bgpkb.pipeline.build_human_review_decision_audit"
    if task_type == "audit_decisions":
        return "python3 -m bgpkb.pipeline.build_human_review_progress"
    if task_type == "apply_decisions":
        return "python3 -m bgpkb.pipeline.run_pipeline"
    return "python3 -m bgpkb.pipeline.quality_check"
...
def write_report(records):
    ...
    lines = [
        ...
        f"- JSONL 输出：`data/derived/datasets/human_review_handoff.jsonl`",
        f"- CSV 输出：`data/derived/datasets/human_review_handoff.csv`",
        "",
        "## 按任务类型统计",
        "",
    ]
