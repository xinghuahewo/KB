...
REPORT = paths.report_path("human_review_progress_report")
...
def next_step_for(status_counts, decision_counts):
    if decision_counts["ready_to_apply_count"]:
        return "运行 src/bgpkb/pipeline/apply_human_review_decisions.py 显式应用已审计通过的 approved/rejected 决策。"
    ...
    if status_counts["pending_count"]:
        return "人工核验来源路径，在 data/review_inputs/human_review_decisions.csv 中填写 approved/rejected/needs_source/needs_semantic_review。"
    return "当前范围没有待处理实体。"
    ...
    for (scope_type, scope_value), group in sorted(grouped.items()):
        ...
        records.append({
            ...
            **decision_counts,
            ...
            "generated_by": "src/bgpkb/pipeline/build_human_review_progress.py",
        })
...
def write_report(records):
    ...
    lines = [
        ...
        f"- JSONL 输出：`data/derived/datasets/human_review_progress.jsonl`",
        f"- CSV 输出：`data/derived/datasets/human_review_progress.csv`",
        "",
        "## 下一步",
        "",
