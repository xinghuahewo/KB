...
REPORT = paths.report_path("human_review_workbook_report")
...
def decision_instructions_for(packet):
    if packet.get("review_bucket") == "ready_without_manual_note":
        return "人工打开 data/corpus/parsed/cleaned/chunk 证据；若实体字段被来源直接支持，可把 review_decision 改为 approved。"
    if packet.get("review_bucket") == "ready_with_manual_note":
...
def build_records():
    ...
    for packet in load_jsonl(DATASET_DIR / "entity_review_packets.jsonl"):
        ...
        record = {
            ...
            "generated_by": "src/bgpkb/pipeline/build_human_review_workbook.py",
        }
...
def write_report(records):
    ...
    lines = [
        ...
        f"- JSONL 输出：`data/derived/datasets/human_review_workbook.jsonl`",
        f"- CSV 输出：`data/derived/datasets/human_review_workbook.csv`",
    ...
    lines.extend([
        ...
        "- 未从论文正文或案例正文抽取新结构化字段。",
        "- 未自动修改 data/knowledge/entities/*.jsonl 中的 review_status。",
    ])
    REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8")
