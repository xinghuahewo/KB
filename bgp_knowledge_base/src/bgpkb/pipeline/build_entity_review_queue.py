...
REPORT = paths.report_path("entity_review_queue_report")
...
def build_item(record, source_statuses):
    ...
    return {
        ...
        "generated_by": "src/bgpkb/pipeline/build_entity_review_queue.py",
    }
...
def write_report(records):
    ...
    lines = [
        ...
        "## 范围",
        "",
        "本报告从 `data/knowledge/entities/*.jsonl` 和 `data/derived/datasets/source_processing_status.jsonl` 机械生成待复核队列。该步骤不改变实体审核状态，不判断定义是否正确，只标出来源处理状态是否足以进入人工复核。",
        "",
        "## 摘要",
        "",
        f"- 队列记录数：{len(records)}",
        f"- JSONL 输出：`data/derived/datasets/entity_review_queue.jsonl`",
        f"- CSV 输出：`data/derived/datasets/entity_review_queue.csv`",
        "",
        "## 按建议动作统计",
        "",
    ]
