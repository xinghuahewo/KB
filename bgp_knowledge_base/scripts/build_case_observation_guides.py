...
REPORT_DIR = ROOT / "reports" / "case_observation_guides"
...
def write_readme(grouped, source_by_id, skipped_actions):
    ...
    for source_id, records in grouped.items():
        ...
        lines.append(
            f"- `reports/case_observation_guides/{filename}`："
...
def write_case_file(source_id, records, source):
    ...
    source_ref = records[0].get("source_ref", "") if records else f"cleaned/cases/{source_id}.md"
