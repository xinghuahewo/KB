...
REPORT = ROOT / "reports" / "case_observation_report.md"
...
def cleaned_case_path(source_id):
    return ROOT / "cleaned" / "cases" / f"{source_id}.md"
    ...
    for observation_type, pattern in OBSERVATION_PATTERNS:
        for match in pattern.finditer(text):
            ...
            records.append({
                ...
                "source_ref": f"cleaned/cases/{source_id}.md",
...
def write_report(records, case_statuses, total_cases):
    ...
    lines = [
        ...
        f"- 观察值总数：{len(records)}",
        f"- JSONL 输出：`datasets/case_observations.jsonl`",
        f"- CSV 输出：`datasets/case_observations.csv`",
        "",
        "## 按观察类型统计",
        "",
    ]
