...
REPORT = ROOT / "reports" / "published_knowledge_base_report.md"

ENTITY_DIR = ROOT / "entities"
CHUNK_DIR = ROOT / "chunks"
DATASET_DIR = ROOT / "datasets"
RELATIONSHIP_FILE = ROOT / "relationships" / "relationships.jsonl"
SOURCE_INVENTORY = ROOT / "inventory" / "sources.csv"
...
def write_readme(manifest):
    lines = [
        ...
        "",
        "本目录是从 `raw/`、`parsed/`、`cleaned/`、`chunks/`、`entities/`、`relationships/` 和 `datasets/` 机械汇总出的发布入口。",
        "",
        ...
        "- `bgp_knowledge_base.sqlite`：由 `scripts/build_sqlite_knowledge_base.py` 生成的本地 SQL 查询入口。",
        ...
        "- 人工复核时可通过 SQLite 查询 `human_review_evidence_extracts` 或 `scripts/query_knowledge_base.py extracts <entity_id>` 查看确定性 chunk 摘录。",
        "- 人工分批复核时可通过 SQLite 查询 `human_review_session_queue` 或 `scripts/query_knowledge_base.py sessions --session-id <session_id>` 查看会话队列。",
...
def main():
    ...
    manifest = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "generated_by": "scripts/build_published_knowledge_base.py",
        "inputs": [
            ...
            "datasets/entity_review_packets.jsonl",
            "datasets/source_processing_status.jsonl",
            "datasets/next_action_queue.jsonl",
            "datasets/human_review_decision_audit.jsonl",
            "datasets/human_review_decision_apply_preview.jsonl",
            "datasets/human_review_input_validation.jsonl",
            "datasets/human_review_progress.jsonl",
            "datasets/human_review_field_checklist.jsonl",
            "datasets/human_review_source_matrix.jsonl",
            "datasets/human_review_task_board.jsonl",
            "datasets/human_review_handoff.jsonl",
        ],
        "outputs": [
            "published/README.md",
