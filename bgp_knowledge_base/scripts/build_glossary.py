...
REPORT = ROOT / "reports" / "glossary_report.md"
...
def write_report(records):
    ...
    lines = [
        ...
        "## 范围",
        "",
        "本报告从 `entities/*.jsonl` 机械派生术语表，只搬运已有实体字段，不新增语义解释、不做同义词推断、不改变审核状态。",
        "",
        "## 摘要",
        "",
        f"- 术语记录数：{len(records)}",
        f"- JSONL 输出：`datasets/glossary.jsonl`",
        f"- CSV 输出：`datasets/glossary.csv`",
        "",
        "## 按实体类型统计",
        "",
    ]
