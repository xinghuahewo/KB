...
REPORT = ROOT / "reports" / "source_gap_queue_report.md"
...
def build_item(record):
    ...
    return {
        ...
        "generated_by": "scripts/build_source_gap_queue.py",
    }
...
def write_report(records):
    ...
    lines = [
        ...
        "## 范围",
        "",
        "本报告从 `datasets/source_processing_status.jsonl` 机械生成来源层缺口队列。该步骤不联网、不归档缺失来源，只把未完成来源转成后续动作清单。",
        "",
        "## 摘要",
        "",
        f"- 队列记录数：{len(records)}",
        f"- JSONL 输出：`datasets/source_gap_queue.jsonl`",
        f"- CSV 输出：`datasets/source_gap_queue.csv`",
        "",
        "## 按缺口类型统计",
        "",
    ]
