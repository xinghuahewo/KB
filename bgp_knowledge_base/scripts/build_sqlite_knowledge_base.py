...
REPORT = ROOT / "reports" / "sqlite_knowledge_base_report.md"
...
def insert_meta(conn, manifest, fts_enabled):
    rows = [
        ("generated_by", "scripts/build_sqlite_knowledge_base.py"),
...
def build_database():
    ...
    entity_evidence = load_jsonl(ROOT / "datasets" / "entity_source_evidence.jsonl")
    review_packets = load_jsonl(ROOT / "datasets" / "entity_review_packets.jsonl")
    next_actions = load_jsonl(ROOT / "datasets" / "next_action_queue.jsonl")
    case_observations = load_jsonl(ROOT / "datasets" / "case_observations.jsonl")
    glossary = load_jsonl(ROOT / "datasets" / "glossary.jsonl")
    human_review_workbook = load_jsonl(ROOT / "datasets" / "human_review_workbook.jsonl")
    human_review_decision_audit = load_jsonl(ROOT / "datasets" / "human_review_decision_audit.jsonl")
    human_review_decision_apply_preview = load_jsonl(ROOT / "datasets" / "human_review_decision_apply_preview.jsonl")
    human_review_input_validation = load_jsonl(ROOT / "datasets" / "human_review_input_validation.jsonl")
    human_review_progress = load_jsonl(ROOT / "datasets" / "human_review_progress.jsonl")
    human_review_evidence_extracts = load_jsonl(ROOT / "datasets" / "human_review_evidence_extracts.jsonl")
    human_review_session_queue = load_jsonl(ROOT / "datasets" / "human_review_session_queue.jsonl")
    human_review_session_status = load_jsonl(ROOT / "datasets" / "human_review_session_status.jsonl")
    human_review_field_checklist = load_jsonl(ROOT / "datasets" / "human_review_field_checklist.jsonl")
    human_review_source_matrix = load_jsonl(ROOT / "datasets" / "human_review_source_matrix.jsonl")
    human_review_task_board = load_jsonl(ROOT / "datasets" / "human_review_task_board.jsonl")
    human_review_handoff = load_jsonl(ROOT / "datasets" / "human_review_handoff.jsonl")
