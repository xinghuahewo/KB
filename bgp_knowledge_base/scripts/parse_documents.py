...
REPORT = ROOT / "reports" / "parse_report.md"
...
def process_file(path, parsed_subdir, cleaned_subdir):
    ...
    write_outputs(
        doc,
        text,
        ROOT / "parsed" / parsed_subdir / f"{doc_id}.json",
        ROOT / "cleaned" / cleaned_subdir / f"{doc_id}.md",
    )
    return {
...
def main():
    targets = []
    targets.extend((path, "standards", "standards") for path in sorted((ROOT / "raw" / "standards").glob("*.txt")))
    targets.extend((path, "data_docs", "data_docs") for path in sorted((ROOT / "raw" / "data_docs").glob("*")))
    targets.extend((path, "data_docs", "data_docs") for path in sorted((ROOT / "raw" / "tools_docs").glob("*")))
    targets.extend((path, "papers", "papers") for path in sorted((ROOT / "raw" / "papers").glob("*")))
    targets.extend((path, "cases", "cases") for path in sorted((ROOT / "raw" / "cases").glob("*")))
