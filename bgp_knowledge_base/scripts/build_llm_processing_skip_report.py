...
REPORT = ROOT / "reports" / "llm_processing_skip_report.md"
NEXT_ACTION_QUEUE = ROOT / "datasets" / "next_action_queue.jsonl"
...
STATIC_SKIP_ITEMS = [
    ...
    (
        ...
        "已生成规则观察值 `datasets/case_observations.*`；结构化实体写入仍等待人工审阅",
    ),
    (
        "source-derived chunks 到实体关系的自动推断",
        "需要语义匹配和关系判断",
        "暂不扩展 relationships/relationships.jsonl",
    ),
    (
    ...
    (
        ...
        "已生成 `datasets/artifact_manifest.*`；主题充分性仍等待人工或明确允许的语义流程",
    ),
    (
        "术语表别名补全和定义润色",
        "判断同义词、缩写边界和更优定义需要语义判断",
        "已生成机械派生的 `datasets/glossary.*`；新增别名和定义改写等待人工或明确允许的语义流程",
    ),
    (
        "根据复核队列自动审批实体",
        "判断实体定义是否准确、证据是否充分需要人工或语义判断",
        "已生成 `datasets/entity_review_queue.*` 供人工复核；实体仍保持 `pending`",
    ),
    (
        "判断已归档来源是否足以支持实体批准",
        "来源已完成确定性归档、解析、清洗和切分，但“内容是否充分支持实体字段”需要语义判断",
        "已生成 `datasets/entity_review_queue.*`、`datasets/entity_review_packets.*` 和人工复核指南；实体仍保持 `pending`",
    ),
]
