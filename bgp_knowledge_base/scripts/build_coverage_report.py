...
REPORT = ROOT / "reports" / "coverage_report.md"
...
DATASET_DESCRIPTIONS = {
    "case_observations.jsonl": "从 cleaned/cases 用正则抽取的案例观察值",
    "case_observations.csv": "同一观察值数据的 CSV 版本",
    "source_processing_status.jsonl": "按 inventory/source_id 汇总的确定性处理状态",
...
def relationship_count():
    return len(load_jsonl(ROOT / "relationships" / "relationships.jsonl"))
...
def dataset_rows():
    ...
    for filename, description in DATASET_DESCRIPTIONS.items():
        ...
        rows.append((f"datasets/{filename}", count_dataset(path), description))
    return rows


def source_gap_summary():
    gaps = load_jsonl(ROOT / "datasets" / "source_gap_queue.jsonl")
...
def main():
    ...
    source_statuses = load_jsonl(ROOT / "datasets" / "source_processing_status.jsonl")
    ...
    case_observations = len(load_jsonl(ROOT / "datasets" / "case_observations.jsonl"))
    ...
    for dirname in ("standards", "data_docs", "papers", "cases"):
        lines.append(f"| parsed/{dirname} | {parsed_counts.get(dirname, 0)} 个 JSON |")
    for dirname in ("standards", "data_docs", "papers", "cases", "notes"):
        count = cleaned_counts.get(dirname, 0)
        if count:
            lines.append(f"| cleaned/{dirname} | {count} 个 Markdown |")
    ...
    for filename in [
        ...
        lines.append(f"| published/{filename} | {status} |")
    if published_counts:
        lines.extend(["", "| 发布项 | 数量 |", "| --- | ---: |"])
