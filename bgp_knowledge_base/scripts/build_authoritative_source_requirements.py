...
REPORT = ROOT / "reports" / "authoritative_source_requirements_report.md"
...
def build_records():
    ...
    for packet in packets:
        ...
        records.append({
            ...
            "generated_by": "scripts/build_authoritative_source_requirements.py",
        })
...
def write_report(records):
    ...
    lines = [
        ...
        "",
        f"- 需求记录数：{len(records)}",
        f"- JSONL 输出：`datasets/authoritative_source_requirements.jsonl`",
        f"- CSV 输出：`datasets/authoritative_source_requirements.csv`",
