...
REVIEW_INPUT_DIR = ROOT / "review_inputs"
DECISION_INPUT = REVIEW_INPUT_DIR / "human_review_decisions.csv"
REPORT = ROOT / "reports" / "human_review_decision_audit_report.md"
...
def build_records():
    ...
    for workbook in load_jsonl(DATASET_DIR / "human_review_workbook.jsonl"):
        ...
        audit_source = "review_inputs/human_review_decisions.csv" if input_decision else "workbook_default"
        ...
        records.append({
            ...
            "generated_by": "scripts/build_human_review_decision_audit.py",
        })
...
def write_report(result):
    ...
    lines = [
        ...
        f"- JSONL 输出：`datasets/human_review_decision_audit.jsonl`",
        f"- CSV 输出：`datasets/human_review_decision_audit.csv`",
        f"- 人工决策输入：`review_inputs/human_review_decisions.csv`",
        "",
        "## 按应用状态统计",
        "",
    ]
