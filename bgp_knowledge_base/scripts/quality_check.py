...
REPORT_FILE = ROOT / "reports" / "quality_report.md"
...
CLEANED_MANUAL_NOTES = {
    "cleaned/notes/context_summary.md": "context_2026",
}
...
def load_sources():
    path = ROOT / "inventory" / "sources.csv"
...
def parse_output_dirs(raw_path):
    ...
    if rel.startswith("raw/standards/"):
        return "standards", "standards"
    if rel.startswith("raw/data_docs/") or rel.startswith("raw/tools_docs/"):
        return "data_docs", "data_docs"
    if rel.startswith("raw/papers/"):
        return "papers", "papers"
    if rel.startswith("raw/cases/"):
        return "cases", "cases"
    return None, None
...
def main():
    ...
    for row in source_rows:
        ...
        try:
            ...
            continue
        if rel_path.as_posix().startswith("raw/"):
            ...
            if is_parseable_raw_path(rel_path):
                ...
                if parsed_subdir and not (ROOT / "parsed" / parsed_subdir / f"{rel_path.stem}.json").exists():
                    inventory_parseable_missing_parsed.append(f"{source_id} -> {rel_path.as_posix()}")
                if cleaned_subdir and not (ROOT / "cleaned" / cleaned_subdir / f"{rel_path.stem}.md").exists():
    ...
    review_input_path = ROOT / "review_inputs" / "human_review_decisions.csv"
    ...
    for index, record in enumerate(human_review_decision_audit_items, start=1):
        ...
        if entity_id:
            ...
            if workbook is None:
                ...
                expected_source = "review_inputs/human_review_decisions.csv" if input_decision else "workbook_default"
    ...
    for index, record in enumerate(human_review_input_validation_items, start=1):
        ...
        if record.get("input_path") != "review_inputs/human_review_decisions.csv":
    ...
    for index, record in enumerate(human_review_task_board_items, start=1):
        ...
        if task_type == "review_session":
            ...
            expected_command = f"python3 scripts/import_human_review_session_decisions.py --session-id {session_id}"
            ...
            if task_type == "validate_input" and record.get("suggested_command") != "python3 scripts/build_human_review_input_validation.py":
                human_review_task_board_command_mismatches.append(f"{record_label} validate command")
            if task_type == "audit_decisions" and record.get("suggested_command") != "python3 scripts/build_human_review_decision_audit.py":
            ...
            if task_type == "apply_decisions" and record.get("suggested_command") != "python3 scripts/apply_human_review_decisions.py":
                human_review_task_board_command_mismatches.append(f"{record_label} apply suggested_command")
            if task_type == "apply_decisions" and record.get("write_command") != "python3 scripts/apply_human_review_decisions.py --write":
    ...
    template_dir = ROOT / "review_inputs" / "human_review_session_decision_templates"
    ...
    if not (template_dir / "README.md").exists():
        human_review_session_decision_template_missing_files.append("review_inputs/human_review_session_decision_templates/README.md")
