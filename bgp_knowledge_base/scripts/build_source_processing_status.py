...
REPORT = ROOT / "reports" / "source_processing_status_report.md"
...
def load_chunk_counts():
    counts = Counter()
    for path in (ROOT / "chunks").glob("*.jsonl"):
    ...
    if not source_path:
        ...
        if not resolved.exists():
            ...
            if parseable:
                ...
                parsed_status = "present" if (ROOT / "parsed" / parsed_subdir / f"{rel_path.stem}.json").exists() else "missing"
                cleaned_status = "present" if (ROOT / "cleaned" / cleaned_subdir / f"{rel_path.stem}.md").exists() else "missing"
            else:
...
def write_report(records):
    ...
    lines = [
        ...
        "## 范围",
        "",
        "本报告按 `inventory/sources.csv` 的 source_id 汇总确定性处理状态，只根据文件存在性和已生成产物统计，不进行语义判断。",
        "",
        "## 摘要",
        "",
        f"- 来源总数：{len(records)}",
        f"- JSONL 输出：`datasets/source_processing_status.jsonl`",
        f"- CSV 输出：`datasets/source_processing_status.csv`",
        "",
        "## 按处理状态统计",
        "",
    ]
