...
REPORT = ROOT / "reports" / "entity_review_packet_report.md"
...
def suggested_action_for(bucket):
    ...
    if bucket == "ready_without_manual_note":
        return "优先人工核验 cleaned/parsed/chunk 证据，确认后再决定 approved/rejected。"
...
def build_packets():
    ...
    for entity in load_entities():
        ...
        packet = {
            ...
            "generated_by": "scripts/build_entity_review_packets.py",
        }
...
def write_report(records):
    ...
    lines = [
        ...
        f"- JSONL 输出：`datasets/entity_review_packets.jsonl`",
        f"- CSV 输出：`datasets/entity_review_packets.csv`",
