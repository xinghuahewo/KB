...
REPORT = ROOT / "reports" / "human_review_decision_apply_report.md"
...
def write_report(decision_count, skipped, updated, unchanged, missing, by_file, write=False):
    lines = [
        ...
        f"- JSONL 输出：`datasets/human_review_decision_apply_preview.jsonl`",
        f"- CSV 输出：`datasets/human_review_decision_apply_preview.csv`",
        "",
        "## 按文件更新数",
        "",
    ]
...
def build_preview_records(decision_count, skipped, updated, unchanged, missing, write=False):
    ...
    records = [
        {
            ...
            "generated_by": "scripts/apply_human_review_decisions.py",
        }
    ]
    for status, count in sorted(skipped.items()):
        records.append({
    ...
    for index, entity_id in enumerate(missing, start=1):
        records.append({
            ...
            "needs_llm": False,
            "count": 1,
            "message": f"{entity_id} 在 entities/*.jsonl 中未找到。",
            "generated_by": "scripts/apply_human_review_decisions.py",
        })
    return records
...
def main():
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="Actually modify entities/*.jsonl. Without this flag the script only writes a dry-run report.",
    )
