...
from service import retrieval_framework
...
EVAL = ROOT / "datasets" / "rag_query_eval.jsonl"
...
def build_eval_records():
    ...
    for query in QUERIES:
        ...
        records.append({
            ...
            "generated_by": "scripts/build_rag_readiness_report.py",
        })
    return records
...
def main():
    ...
    manifest = retrieval_framework.load_json(ROOT / "published" / "embedding_manifest.json", default={})
    retrieval_index = retrieval_framework.load_json(ROOT / "published" / "rag_retrieval_index.json", default={})
    ...
    lines = [
        ...
        "## RAG 索引覆盖",
        "",
        f"- Embedding manifest：`published/embedding_manifest.json`",
