...
REPORT = ROOT / "reports" / "human_review_evidence_extracts_report.md"
    ...
    for workbook in workbook_records:
        ...
        for rank, (score, chunk_id, matched_terms, chunk) in enumerate(scored_chunks[:MAX_EXTRACTS_PER_ENTITY], start=1):
            ...
            records.append({
                ...
                "generated_by": "scripts/build_human_review_evidence_extracts.py",
            })
...
def write_report(records, missing_chunks):
    ...
    lines = [
        ...
        f"- JSONL 输出：`datasets/human_review_evidence_extracts.jsonl`",
        f"- CSV 输出：`datasets/human_review_evidence_extracts.csv`",
        "",
        "## 按实体类型统计",
        "",
    ]
