...
- [ ] 切到 `main` 并快进合并阶段 4.4。
- [ ] 推送 `main`。
- [ ] 新建 `codex/phase-4-5-aliyun-bge-m3-hybrid`。
- [ ] 运行 `python3 -m pytest -v` 确认基线未破坏。

### 任务 2：配置阿里云 BGE-M3 provider 边界

**文件：**
- 修改：`config/rag_retrieval.yaml`
- 测试：`tests/test_embedding_provider.py`

- [ ] 写失败测试：默认不运行本地模型，`aliyun_bge_m3` 只读取环境变量。
- [ ] 写失败测试：无 `ALIYUN_BGE_M3_ENDPOINT` 或 `ALIYUN_BGE_M3_API_KEY` 时 provider 标记为 unavailable。
- [ ] 实现配置字段：provider、model_name、endpoint_env、api_key_env、timeout_seconds、batch_size、enabled。
- [ ] 运行 `python3 -m pytest tests/test_embedding_provider.py -v`。

### 任务 3：实现阿里云 BGE-M3 客户端

**文件：**
- 新建：`service/aliyun_bge_m3_client.py`
- 测试：`tests/test_aliyun_bge_m3_client.py`

- [ ] 写失败测试：客户端 payload 使用 `input` 和 `embedding_type=dense`。
- [ ] 写失败测试：真实 key 不会出现在异常、报告或 repr 中。
- [ ] 写失败测试：fake response 能解析为 `list[float]`。
- [ ] 实现 `AliyunBgeM3Client.embed_texts(texts: list[str])`。
- [ ] 运行 `python3 -m pytest tests/test_aliyun_bge_m3_client.py -v`。

### 任务 4：构建文件化 embedding index

**文件：**
- 新建：`scripts/build_aliyun_bge_m3_index.py`
- 新建：`published/aliyun_bge_m3_vector_index.jsonl`
- 新建：`published/aliyun_bge_m3_embedding_manifest.json`
- 新建：`reports/aliyun_bge_m3_embedding_report.md`
- 测试：`tests/test_build_aliyun_bge_m3_index.py`
...
- [ ] 运行 `python3 -m pytest tests/test_build_aliyun_bge_m3_index.py -v`。
...
- 修改：`README.md`
- 修改：`docs/stages/phase_4_5_aliyun_bge_m3_hybrid_retrieval_v1.md`

- [ ] 新增 `phase_4_5_aliyun_bge_m3_hybrid_retrieval_v1` 阶段验收项。
- [ ] README 增加阿里云 BGE-M3 环境变量和运行命令。
- [ ] 阶段文档更新真实验收结果。
- [ ] 运行 `python3 scripts/run_stage_acceptance.py --stage phase_4_5_aliyun_bge_m3_hybrid_retrieval_v1`。
...
- [ ] 运行 `git diff --check`。
- [ ] 运行密钥扫描，确认 DeepSeek 和阿里云真实 key 未落盘。
- [ ] 提交并推送阶段 4.5 分支。
