...
- 第一版前端只使用 `CHAT_PROVIDER=bgp_fastapi_rag`，不在 `chat_frontend` 中配置模型 API key。
- DeepSeek key 配置在后端 `bgp_knowledge_base/.env`，由 FastAPI RAG 服务读取。
- 若后续接 OpenAI Responses API，只能在 Next.js 服务端读取 `OPENAI_API_KEY`，并且不得暴露给浏览器。
