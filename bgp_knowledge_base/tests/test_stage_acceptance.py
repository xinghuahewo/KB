...
CONFIG = ROOT / "config" / "stage_acceptance_gates.yaml"
REPORT = ROOT / "reports" / "stage_acceptance_report.md"
RESULTS = ROOT / "datasets" / "stage_acceptance_results.jsonl"
SCRIPT = ROOT / "scripts" / "run_stage_acceptance.py"
...
def test_stage_acceptance_config_registers_phase_1_effect_checks():
    ...
    assert "reports/data_management_report.md" in stage["required_files"]
...
def test_stage_acceptance_config_registers_phase_3_5_semantic_identity():
    ...
    assert "config/semantic_identity.yaml" in stage["required_files"]
    assert "published/jsonld_context.json" in stage["required_files"]
    assert "published/semantic_id_map.jsonl" in stage["required_files"]
...
def test_stage_acceptance_config_registers_phase_4_rag_framework():
    ...
    assert "config/rag_retrieval.yaml" in stage["required_files"]
    assert "config/llm_candidate_enrichment.yaml" in stage["required_files"]
    assert "reports/rag_readiness_report.md" in stage["required_files"]
