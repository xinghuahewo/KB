...
BGP 知识库已经从“可检索的资料集合”升级为“可持续生产、可验证回答、可安全发布的专业知识服务”。本轮完成了来源、数据加工、检索、回答、治理和发布六个环节的闭环，当前版本已经投入生产运行，并保留完整的回退能力。

这次建设的重点不是单次提高黄金集分数，而是形成可长期迭代的产品能力：新资料可以按统一流程进入知识库，低质量内容会被阻断，回答能够追溯到具体证据，模型不能越权批准知识，版本出现问题时可以恢复到上一套完整服务。

### 已形成的核心能力

1. **来源可信且全程可追溯**

   当前 54 个知识来源均已登记并形成不可变快照。每份内容都能追溯到来源、获取状态、许可证状态和原始版本；来源更新不会覆盖历史记录，重复内容也不会产生无法解释的副本。

2. **数据生产流程统一且可持续运行**

   从来源接入、文档标准化、语义加工、检索发布到上线验收已经收敛为一条统一流程。流程支持失败停止、断点恢复和增量重建，不再依赖大量互相独立、需要人工记忆顺序的脚本。

3. **面向 BGP 场景的快速语义检索**

   系统能够处理中文和英文问题，覆盖事实查询、操作过程、策略规范和全局性问题。当前真实问题集的证据召回率为 82.95%，达到首版 80% 发布基线；生产环境 95% 的检索请求在约 215 毫秒内完成，未再出现逐次扫描大型向量文件导致的十几秒等待。

4. **回答可核查，证据不足时会保守拒答**

   回答不再只是附带一组来源，而是把事实主张与实际使用的证据逐项关联。系统会拒绝不存在的引用、没有证据支撑的事实和不符合结构要求的模型输出；修复仍失败时返回证据不足，而不会把自由文本包装成可靠答案。

5. **外部资料和模型输出受到治理约束**

   解析质量、内容质量、来源可信、语义审核和检索资格已经分开管理。模型只能提出待审核候选，不能直接批准来源、修改检索资格或把生成内容写成正式知识；资料中的提示注入文本也按不可信内容处理。

6. **发布可验证、可灰度、可成对回滚**

   每次发布都先在隔离候选环境完成数据质量、真实检索、真实回答、性能和兼容性验收，再进行生产切换。代码和知识数据作为一对发布和回滚，上一版本持续保留，出现严重问题时不需要重新构建数据即可恢复服务。

7. **现有使用方式保持稳定**

   现有网页入口、问答接口、流式进度和多引用展示继续可用，没有引入账号体系或云端同步，也没有改变当前服务器运行方式。大型语料、数据库和向量索引仍作为外部发布资产管理，没有重新放入普通 Git 历史。

### 当前能力指标

| 能力维度 | 当前基线 | 业务含义 |
| --- | ---: | --- |
| 来源覆盖 | 54/54 | 所有现有生产来源均有明确身份和处理结果 |
| 可检索证据单元 | 2,640 | 已从旧语料碎片收敛为可追溯的语义证据 |
| 真实检索题覆盖 | 104 题 | 同时覆盖中英文、事实、过程、政策、全局和困难负例 |
| Recall@8 | 82.95% | 达到首版 80% 发布标准，未继续针对当前黄金集过拟合 |
| 检索时延 p95 | 约 215ms | 绝大多数检索可在交互等待范围内完成 |
| 困难负例拒答率 | 100% | 明显超出知识范围的问题不会强行生成答案 |
| 结构化回答验收 | 24 题 | 覆盖多引用、证据不足、来源冲突和提示注入 |
| 生产状态 | 已上线、无降级 | 当前版本正常服务，上一版本可直接回滚 |

### 当前边界与下一阶段重点

- 这是面向 BGP 专业知识的证据型问答系统，不承担实时互联网搜索、新闻追踪或通用聊天能力。
- 82.95% 召回率是首版发布基线，不是长期质量目标。后续提升应依靠新增高质量来源、通用切块与检索改进以及独立盲测集，而不是继续围绕当前黄金题定向调参。
- 回答已经具备逐条证据约束，但复杂长答案的证据覆盖深度仍是下一阶段最值得提升的产品指标。
- 旧版兼容入口目前只用于历史追溯，生产服务已不再依赖；其彻底退役已拆分为独立变更，实施时仍需保留历史版本、迁移证据和审计记录。

### 验收与追溯

最终版本已通过完整代码回归、真实服务器检索与回答评测、隔离验证、生产切换和回滚演练。详细命令、任务映射、门禁阈值、制品校验和报告摘要保留在本文件前述各检查点、`openspec/changes/rag-evidence-pipeline-v2/tasks.md`、ADR 0006/0007 以及独立的 v1 退役提案中，供后续审计和问题追溯使用。
