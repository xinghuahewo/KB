...
REPORT = paths.report_path("next_action_queue_report")
...
def action_for_requirement(record):
    ...
    return {
        ...
        "related_dataset": "data/derived/datasets/authoritative_source_requirements.jsonl",
        "blocking_reason": "实体目前仅有 context_2026/manual_note 来源，不能进入批准复核。",
        "suggested_action": record["suggested_action"],
        "needs_llm": False,
        "skip_reason": "",
        "generated_by": "src/bgpkb/pipeline/build_next_action_queue.py",
    }
...
def action_for_packet(record):
    ...
    return {
        ...
        "related_dataset": "data/derived/datasets/entity_review_packets.jsonl",
    ...
    return [
        {
            ...
            "source_refs": [],
            "related_dataset": "data/knowledge/entities/papers.jsonl",
        ...
        },
        {
            ...
            "review_bucket": "semantic_skip",
            "source_refs": [],
            "related_dataset": "data/derived/datasets/case_observations.jsonl",
            "blocking_reason": "案例观察值已有 148 条，但事件角色、证据强度和影响范围需要语义判断。",
            "suggested_action": "明确允许语义流程后，再决定是否写入 data/knowledge/entities/cases.jsonl 或扩展案例字段。",
            "needs_llm": True,
...
def write_report(records):
    ...
    lines = [
        ...
        "",
        f"- 行动记录数：{len(records)}",
        f"- JSONL 输出：`data/derived/datasets/next_action_queue.jsonl`",
        f"- CSV 输出：`data/derived/datasets/next_action_queue.csv`",
