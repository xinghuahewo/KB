...
EVIDENCE_FILE = paths.DATASETS_DIR / "entity_source_evidence.jsonl"
PACKET_FILE = paths.DATASETS_DIR / "entity_review_packets.jsonl"
ACTION_FILE = paths.DATASETS_DIR / "next_action_queue.jsonl"
DECISION_AUDIT_FILE = paths.DATASETS_DIR / "human_review_decision_audit.jsonl"
...
def render_report(config, rows, checks):
    ...
    lines = [
        ...
        "## 范围",
        "",
        "本报告基于 `metadata/config/lifecycle_policy.yaml` 生成，读取现有实体、证据索引、复核包、行动队列和人工决策审计，输出实体级生命周期治理视图。",
        "",
