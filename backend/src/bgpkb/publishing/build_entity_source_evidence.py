...
REPORT = paths.report_path("entity_source_evidence_report")
...
def parsed_path_for(source_id):
    matches = sorted((paths.PARSED_DIR).glob(f"*/{source_id}.json"))
    if matches:
...
def cleaned_path_for(source_id):
    matches = sorted((paths.CLEANED_DIR).glob(f"*/{source_id}.md"))
    if matches:
        return matches[0].relative_to(ROOT).as_posix()
    return ""
...
def write_report(records):
    ...
    lines = [
        ...
        f"- JSONL 输出：`data/derived/datasets/entity_source_evidence.jsonl`",
        f"- CSV 输出：`data/derived/datasets/entity_source_evidence.csv`",
