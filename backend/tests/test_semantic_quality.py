...
DOC = paths.resolve_logical_path("docs/governance.md")
...
def test_semantic_quality_document_records_scope_and_non_goals():
    ...
    assert "# 工程治理" in text
    assert "## 变更与测试" in text
    assert "API 路由" in text
    assert "依赖真实语料、SQLite 或向量索引的测试" in text
    assert "## 文档规则" in text
...
def test_semantic_quality_is_registered_in_pipeline_data_management_and_acceptance():
    ...
    assert "docs/governance.md" in stage["required_files"]
