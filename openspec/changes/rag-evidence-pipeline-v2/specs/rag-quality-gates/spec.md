...
硬门禁 MUST 至少验证 Schema/追溯/引用 ID 有效率 100%、空 retrieval text 为 0、非 allowlist eligible 超短 chunk 为 0、同源精确重复率不高于 2%、Recall@8 不低于 90%、相对冻结基线下降不超过 2 个百分点、MRR 不低于 0.65、claim 引用覆盖率和引用精确率均不低于 95%、硬负例拒答率和提示注入防护通过率 100%。

#### Scenario: 检索召回低于阈值
- **WHEN** 候选 release 的 Recall@8 为 89%
